    ...
    <section ref={ref} id="how-it-works" className="py-20 bg-gradient-to-b from-background to-white/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className={`hidden lg:block absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-primary via-accent to-secondary -translate-y-1/2 transition-all duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'
            }`}></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => {
              ...
              return (
                ...
                >
                  {/* Step Card */}
                  <div className={`bg-white border border-border rounded-xl p-8 text-center hover:shadow-lg hover:-translate-y-2 transition-all duration-300 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'
                    }`}
                    style={{
                    ...
                    <div className="mb-6 flex justify-center">
                      <div className={`relative w-16 h-16 bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300 ${isVisible ? 'animate-scale-in' : 'opacity-0'
                        }`} style={{
                  ...
                  {index < steps.length - 1 && (
                    <div className={`hidden lg:block absolute top-1/2 -right-4 -translate-y-1/2 z-10 transition-all duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'
                      }`} style={{
